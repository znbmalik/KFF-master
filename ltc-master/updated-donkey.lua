...
local function loadRGB(path, set)
    ...
    local matched = _videoName:match('.avi_%d%d%d%d')--checking format only thay is there multiple clips or not
    
    local video
    if(matched) then
        print(_videoName)
        print(string.sub(_videoName, 1, -1) ..'.t7')
        --video = torch.load(paths.concat(opt.framesRoot, _className, string.sub(_videoName:match("(.*".. '_'..")"), 1, -2) ..'.t7'))
        video = torch.load(paths.concat(opt.framesRoot, _className, string.sub(_videoName, 1, -1) ..'.t7'))
    else
