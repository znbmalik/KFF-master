...
cd '../datasets/Full-Flow-KFF-JHMDB/splits';
%cd '../datasets/RGB-Datasets/Full-Flow-Peak-UCF101/splits';
for sp = 1:1
    ...
    p_train = textread(['../annot/ucfTrainTestlist/trainlist0' num2str(sp) '.txt'], '%s');
    for i=1:length(p_train)
        disp([num2str(i) '/' num2str(length(p_train)) ' done.']);
        system(['touch split' num2str(sp) '/train/' p_train{i}]);
    end
    
    p_test = textread(['../annot/ucfTrainTestlist/testlist0' num2str(sp) '.txt'], '%s');
