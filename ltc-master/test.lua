...
function test()
    ...
    if(not opt.evaluate) then
        ...
        print(string.format('Epoch: [%d][TESTING SUMMARY] Total Time(s): %.2f \t Loss: %.2f \t Acc: %.2f\n',
            epoch, timer:time().real, loss, acc))
                 ...
                 -- create the matrix --CM by zainab
	...
	print("Confusion matrix----------------")
    	local file,err = io.open("confusionMatrix.txt",'w')
        if file then
            for i=1,nClasses do
                local total_sample=0
            for j=1,nClasses do
                total_sample=total_sample+cm[i][j]
            end
      		for j=1,nClasses do
		        --file:write(tostring(cm[i][j])/total_sample*100, " ")
                file:write(string.format("%.3f ", cm[i][j]/total_sample*100))
            end
            file:write("\n")
            end
	    file:close()
...
function testBatch(inputsCPU, labelsCPU, indicesCPU)
    ...
    for i=1,scoresCPU:size(1) do
	    ...
	    cm[gt][pred]=cm[gt][pred]+1 --CM by zainab
