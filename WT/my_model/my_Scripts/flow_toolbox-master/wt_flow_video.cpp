...
static Vec3b computeColor(float fx, float fy)
{
    ...
    for (int b = 0; b < 3; b++)
    {
       ...
       //cout<<endl<<"pix value "<<2 - b<<" "<<col<<endl;
...
static void drawOpticalFlow(string name,const Mat_<float>& flowx, const Mat_<float>& flowy, Mat& dst, float maxmotion = -1)
{
...
int c=0;
    string fname=name+"_color_wd.txt";//zz
    ofstream file;//zz
    file.open(fname.c_str());//zz
    for (int y = 0; y < flowx.rows; ++y)
    {
        for (int x = 0; x < flowx.cols; ++x)
        {
            ...
            //cout<<endl<<"pix value "<<dst.at<Vec3b>(y, x) <<endl;//zz
            file <<dst.at<Vec3b>(y, x);//zz
            c++;//zz
        }
        file << endl;//zz
    }
    file.close();//zz
    //cout<<endl<<"total values "<<c<<endl;
    
    
    
    //modified by zz
    cout<<"weight direction"<<endl;//modified by zz
    ...
    for (int y = 0; y < flowx.rows; ++y)
     {
       for (int x = 0; x < flowx.cols; ++x)
       {
            //    cout<<rad(y,x)<<"\t";
           //rad = 
           //cout<<sqrt((flowx(y, x)/ maxrad) * (flowx(y, x)/ maxrad) + (flowy(y, x) / maxrad) * (flowy(y, x) / maxrad))<<endl;
           //cout<<atan2(-(flowy(y, x)/ maxrad), -(flowx(y, x) / maxrad)) / (float) CV_PI;
           file1 <<sqrt((flowx(y, x)/ maxrad) * (flowx(y, x)/ maxrad) + (flowy(y, x) / maxrad) * (flowy(y, x) / maxrad))*(atan2(-(flowy(y, x)/ maxrad), -(flowx(y, x) / maxrad)) / (float) CV_PI)<<" ";
       }
        file1 << endl;
    }
    file1.close();
...
static void showFlow(string name, const GpuMat& d_flow)
{
    GpuMat planes[2];
    ...
    drawOpticalFlow(name, flowx, flowy, out, -1);
    //cout<<name<<endl;
    imwrite(name+".jpg", out);
    //imshow(name, out);
}

static void showFlow(string name, const Mat& d_flow)
{
	vector<Mat> planes;
    split(d_flow, planes);
    Mat flowx(planes[0]);
...
//fourcc check for image sequence detection, but not sure
    ...
    if(proc_type.compare("gpu") == 0)
    {
    	...
    	cout << "Initialization (this may take awhile)...Ok" << endl;
    	...
    	if(cap.isOpened())
    	{
        	...
        	for(int k=1; k<interval_end-interval_beg+1; k++)
        	{
				...
				string rawname = outname.substr(0, lastindex);
				//cout<<"-------------"<<rawname<<"--------------"<<endl;
				sprintf(name, "%s%s_%d", out_dir.c_str(), rawname.c_str(), k+interval_beg-1);
            	//cout<<"*****************"<<name<<"*******************"<<endl;
			...
			//vector<double> mm_frame = writeFlowJpg(name, g_flow);
                        ...
                        writeFlowMergedJpg(name, g_flow);
                	if(visualize)
                	{
                		showFlow(name, g_flow);
                		//waitKey(30);
                	}
                	//commented by Zainab
